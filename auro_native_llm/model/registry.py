...
MODELS: tuple[ModelProfile, ...] = (
    ...
    ModelProfile("Auro-156K", "micro", "architecture and routing experiments", "decoder-only MoE", True, 1024, ("sensus", "memory_keeper"), ("GQA", "RoPE", "RMSNorm", "SwiGLU", "MoE", NEURO_FEATURE, "constitutional-checkpoints"), "architecture-target", "neuromorphic residual and MoE are architecture paths; no trained quality claim without exact checkpoint evidence"),
    ModelProfile("Auro-2B", "edge", "local assistants, memory, lightweight browser planning", "decoder-only MoE", True, 8192, ("sensus", "operator", "memory_keeper", "browser_brain"), ("GQA", "RoPE", "RMSNorm", "SwiGLU", "MoE", NEURO_FEATURE, "long-context-curriculum", "checkpoint-receipts"), "architecture-target", "declared context, routing, and neuromorphic residual behavior are not quality or physical-energy claims"),
    ModelProfile("Auro-4B", "specialist", "tool use, coding, quantitative work, browser agents", "decoder-only MoE with structured residuals", True, 65536, ("mathesis", "architect", "red_team", "operator", "researcher", "builder", "browser_brain"), ("GQA", "RoPE", "RMSNorm", "SwiGLU", "MoE", NEURO_FEATURE, "Walsh-Hadamard", "long-context-evidence", "constitutional-checkpoints"), "architecture-target", "active compute and stored expert capacity must be reported separately; neuromorphic quality needs exact checkpoint evidence"),
    ModelProfile("Auro-8B", "general", "research, architecture, multi-agent synthesis", "decoder-only MoE", True, 32768, ("nova", "architect", "red_team", "researcher", "builder"), ("GQA", "RoPE", "RMSNorm", "SwiGLU", "MoE", NEURO_FEATURE, "long-context-curriculum", "tool-routing"), "architecture-target", "requires exact checkpoint and benchmark receipts, including neuromorphic ablations before capability promotion"),
    ModelProfile("Auro-14B", "general-high", "high-quality orchestration and domain adaptation", "decoder-only MoE", True, 65536, ("nova", "mathesis", "architect", "researcher"), ("GQA", "RoPE", "RMSNorm", "SwiGLU", "MoE", NEURO_FEATURE, "DPO-ready", "long-context-evidence", "MCP-capability-checkpoints"), "architecture-target", "alignment, hallucination resistance, and neuromorphic benefits are evaluation claims, not architecture facts"),
...
def model_manifest() -> dict[str, Any]:
    return {
        "schema": "auro.model-feature-registry.v2",
