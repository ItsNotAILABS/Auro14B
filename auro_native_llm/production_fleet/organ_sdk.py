...
class AuroOrganSDK:
    ...
    def manifest(self) -> dict[str, Any]:
        return {"schema": "auro.organ_sdk.v2", "approval_authority": "server", "organs": {
    ...
    def verify_server_approval(self, grant: dict[str, Any], actions: list[dict[str, Any]] | None = None, now_ms: int | None = None) -> bool:
        ...
        if now < not_before or now >= expires: return False
    ...
    def execute(self, action: dict[str, Any], *, approval_grant: dict[str, Any] | None = None) -> dict[str, Any]:
        if not approval_grant or not self.verify_server_approval(approval_grant): raise PermissionError("server-authoritative approval required for execution")
        approved_actions = approval_grant.get("actions", [])
        if _canonical(action) not in {_canonical(item) for item in approved_actions}: raise PermissionError("action is not a member of the signed approval set")
...
def build_server_approval(actions: list[dict[str, Any]], subject: str, approval_id: str, *, ttl_ms: int = 60_000, now_ms: int | None = None, key: str | None = None) -> dict[str, Any]:
    ...
    now = int(time.time() * 1000) if now_ms is None else int(now_ms)
    grant = {"schema": "auro.server-approval.v1", "approval_id": approval_id, "subject": subject, "authority": "server", "actions": actions, "actions_sha256": _actions_sha256(actions), "not_before_ms": now, "expires_at_ms": now + int(ttl_ms)}
