...
type Env={AI:Ai;SESSIONS:DurableObjectNamespace<HIMSession>;CLOUDFLARE_API_TOKEN:string;CLOUDFLARE_ACCOUNT_ID:string;OPERATOR_TOKEN:string;EXECUTION_SECRET:string;WORKERS_AI_MODEL:string;EXECUTION_TTL_SECONDS:string};
...
async function cloudflare(env:Env,p:Proposal){
 ...
 return fetch(url,{method:p.method,headers:{authorization:`Bearer ${env.CLOUDFLARE_API_TOKEN}`,"content-type":"application/json"},body:p.body===undefined?undefined:JSON.stringify(p.body)}).then(async r=>({status:r.status,ok:r.ok,data:await r.json()}));
}
