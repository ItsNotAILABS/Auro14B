...
type Env={AI:Ai;ASSETS:Fetcher;SESSIONS:DurableObjectNamespace<HIMSession>;CLOUDFLARE_API_TOKEN?:string;CLOUDFLARE_ACCOUNT_ID?:string;OPERATOR_TOKEN:string;PUBLIC_CHAT_TOKEN?:string;PUBLIC_CHAT_REQUESTS_PER_MINUTE?:string;EXECUTION_SECRET:string;WORKERS_AI_MODEL:string;EXECUTION_TTL_SECONDS:string;MONAD_RPC_URL:string;PYTHON_ENGINE?:Fetcher;JULIA_ENGINE?:Fetcher};
...
const session=(r:Request,e:Env)=>e.SESSIONS.get(e.SESSIONS.idFromName(r.headers.get("x-session-id")||`unauthenticated:${crypto.randomUUID()}`));

export class HIMSession extends DurableObject<Env>{
 async append(kind:string,value:unknown){const ts=Date.now(),prev=(await this.ctx.storage.get<string>("head"))||"0".repeat(64),hash=await sha256(JSON.stringify({ts,kind,value,prev}));await this.ctx.storage.put(`receipt:${ts}:${crypto.randomUUID()}`,{ts,kind,value,prev,hash});await this.ctx.storage.put("head",hash);return {ts,hash,previous_hash:prev,custody:"local-durable-object-hash-chain",signed:false}}
 async history(){const rows=await this.ctx.storage.list({prefix:"receipt:",reverse:true,limit:50});return {head:await this.ctx.storage.get("head"),receipts:[...rows.values()],custody:"local-durable-object-hash-chain",signed:false}}
 async consume(limit:number,windowMs:number){const now=Date.now();const current=await this.ctx.storage.get<{start:number;count:number}>("quota");const bucket=!current||now-current.start>=windowMs?{start:now,count:0}:current;if(bucket.count>=limit)return {allowed:false,remaining:0,reset_at:bucket.start+windowMs};bucket.count+=1;await this.ctx.storage.put("quota",bucket);return {allowed:true,remaining:Math.max(0,limit-bucket.count),reset_at:bucket.start+windowMs}}
}
...
async function plan(env:Env,message:string){const prompt=`You are HIM's governed Cloudflare operator. Return strict JSON with answer and optional proposal. A proposal is {method,path,query?,body?} using a relative https://api.cloudflare.com/client/v4 path. Prefer read-only discovery. Never claim execution. For mutation, explain impact and propose exactly one bounded operation. User: ${message}`;const out=await env.AI.run(env.WORKERS_AI_MODEL as keyof AiModels,{messages:[{role:"user",content:prompt}],response_format:{type:"json_object"}} as never) as {response?:string};try{return JSON.parse(out.response||"{}")}catch{return {answer:out.response||"Unable to produce a plan."}}}
...
export default {async fetch(request:Request,env:Env):Promise<Response>{
 try{
  ...
  const publicResponse=await handlePublicApi(request,env,stub);if(publicResponse)return publicResponse;
  const bearer=request.headers.get("authorization")||"";
  if(url.pathname.startsWith("/api/")&&(!env.OPERATOR_TOKEN||!safeEqual(bearer,`Bearer ${env.OPERATOR_TOKEN}`)))return json({error:"operator authentication required"},401);
