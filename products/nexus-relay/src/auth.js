...
export async function authenticate(request, env) {
  ...
  if (!token.startsWith("nr_live_") && !token.startsWith("nr_test_")) throw Object.assign(new Error("invalid API key format"), { status: 401 });
  ...
  const principal = await env.DB.prepare(
    `SELECT k.id, k.customer_id, k.name, k.plan, k.active,
            ...
            c.status AS customer_status, c.stripe_customer_id
       FROM api_keys k
  ...
  if (principal.customer_status !== "active") throw Object.assign(new Error("customer account is not active"), { status: 403 });
...
export async function reserveUsage(env, principal, operation, metadata = {}, now = new Date()) {
  ...
  ).bind(eventId, principal.id, principal.customer_id, operation, month, minute, now.toISOString(), JSON.stringify(metadata), monthlyLimit, principal.id, month, monthlyLimit, rpm, principal.id, minute, rpm).run();
  if (Number(result?.meta?.changes || 0) !== 1) throw Object.assign(new Error("request quota or rate limit exceeded"), { status: 429, retryAfter: 60 });
  return { event_id: eventId, period: month, limit: monthlyLimit };
}

export async function completeUsage(env, eventId, metadata = {}) {
  await env.DB.prepare("UPDATE usage_events SET status = 'completed', completed_at = ?, metadata_json = ? WHERE id = ? AND status = 'reserved'")
...
export async function usageSummary(env, principal) {
  ...
  const row = await env.DB.prepare("SELECT COUNT(*) AS requests FROM usage_events WHERE api_key_id = ? AND period = ? AND status = 'completed'").bind(principal.id, month).first();
  const used = Number(row?.requests || 0);
  const limit = Number(principal.monthly_limit || 0);
  return { customer_id: principal.customer_id, customer_status: principal.customer_status, key_name: principal.name, plan: principal.plan, period: month, requests_used: used, request_limit: limit, requests_remaining: limit > 0 ? Math.max(0, limit - used) : null, rate_limit_per_minute: Number(principal.rate_limit_per_minute || 0) };
}
