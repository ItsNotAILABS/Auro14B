...
export async function createPortalSession(env, principal, body = {}) {
  ...
  const payload = await stripeRequest(env, "/billing_portal/sessions", {
    ...
    return_url: body.return_url || env.BILLING_PORTAL_RETURN_URL
  });
  return { id: payload.id, url: payload.url };
}

function entitlementFor(env, priceId) {
  let catalog = {};
  try { catalog = JSON.parse(env.RELAY_PRICE_CATALOG || "{}"); } catch { throw httpError("invalid RELAY_PRICE_CATALOG", 503); }
...
async function synchronizeSubscription(env, object, eventId) {
  ...
  await env.DB.prepare("UPDATE api_keys SET active=?, plan=?, monthly_limit=?, rate_limit_per_minute=? WHERE customer_id=?")
    .bind(enabled ? 1 : 0, entitlement.plan || "developer", Number(entitlement.monthly_limit || 1000), Number(entitlement.rate_limit_per_minute || 30), customerId).run();
  return { customer_id: customerId, enabled, price_id: priceId };
}
...
async function recordInvoice(env, object, eventId) {
  await env.DB.prepare(`INSERT INTO invoices (id,customer_id,stripe_customer_id,status,amount_due,amount_paid,currency,hosted_invoice_url,created_at,last_event_id)
    VALUES (?,?,?,?,?,?,?,?,?,?)
    ON CONFLICT(id) DO UPDATE SET status=excluded.status,amount_due=excluded.amount_due,amount_paid=excluded.amount_paid,hosted_invoice_url=excluded.hosted_invoice_url,last_event_id=excluded.last_event_id`)
...
export async function processStripeWebhook(request, env) {
  ...
  const exists = await env.DB.prepare("SELECT id FROM billing_events WHERE id=?").bind(event.id).first();
  if (exists) return { duplicate: true, event_id: event.id };
  ...
  await env.DB.prepare("INSERT INTO billing_events (id,type,created_at,raw_sha256,result_json) VALUES (?,?,?,?,?)")
    .bind(event.id, event.type, new Date().toISOString(), raw_sha256, JSON.stringify(result)).run();
...
export async function billingSummary(env, principal) {
  ...
  return { subscription: subscription || null, invoices: invoices.results || [] };
}
