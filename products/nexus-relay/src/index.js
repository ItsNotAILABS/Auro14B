...
import { billingSummary, createCheckoutSession, createPortalSession, processStripeWebhook } from "./billing.js";
import { enqueueUsageForBilling, flushBillingOutbox } from "./meter_delivery.js";
...
function allowedOrigin(request, env) {
  ...
  return String(env.ALLOWED_ORIGINS || "").split(",").map((x) => x.trim()).filter(Boolean).includes(origin) ? origin : null;
}
...
function detectMode(contentType, text, requested, pathname = "") {
  ...
  const type = (contentType || "").toLowerCase(), path = pathname.toLowerCase();
...
async function sealRelayReceipt(receipt, env) {
  const withHash = { ...receipt, receipt_sha256: await sha256(stable(receipt)) };
  const signingKey = String(env.RELAY_RECEIPT_SIGNING_KEY || "");
  return {
    ...withHash,
    signature_hmac_sha256: signingKey ? await hmacHex(signingKey, stable(withHash)) : "",
  };
}
...
async function readPublicUrl(args, env, dependencies = {}) {
  ...
  const { response, finalUrl, redirectChain, egress } = await secureFetch(target.href, {
    ...
    request: { headers: { "user-agent": "NEXUS-Relay/0.5 (+hosted-public-web-context; contact=operator)", accept: "text/html,application/xhtml+xml,application/json,text/markdown,text/plain,text/csv,application/rss+xml,application/atom+xml,application/xml;q=0.9,*/*;q=0.5" } },
  });
  ...
  const text = new TextDecoder().decode(buffer), fetchedAt = new Date().toISOString();
  ...
  const normalizedText = typeof normalized.content === "string" ? normalized.content : JSON.stringify(normalized.content ?? normalized);
  const receipt = await sealRelayReceipt({
    schema: "nexus.relay.receipt.v3",
    issuer: "nexus-relay",
    ...
    content_sha256: await sha256(normalizedText),
    fetched_at: fetchedAt,
  }, env);
  const result = { ok: response.ok, ...normalized, text: normalizedText, receipt, cache: { hit: false, key: cacheKey } };
...
async function meteredRead(args, env, principal, surface, dependencies = {}) {
  ...
  try {
    ...
    await completeUsage(env, reservation.event_id, { surface, url_host: target.hostname, receipt_sha256: result.receipt.receipt_sha256 });
    const billingDelivery = await enqueueUsageForBilling(env, principal, reservation.event_id);
    return { ...result, metering: { event_id: reservation.event_id, customer_id: principal.customer_id, plan: principal.plan, status: "completed", stripe_delivery: billingDelivery.status } };
  } catch (error) {
...
export default {
  async fetch(request, env) {
    if (request.method === "OPTIONS") {
      ...
      return new Response(null, { status: 204, headers: { ...responseHeaders(request, env), "access-control-allow-methods": "GET,POST,OPTIONS", "access-control-allow-headers": "content-type,authorization,stripe-signature,x-relay-cron-secret" } });
    }
    const url = new URL(request.url);
    if (url.pathname === "/" || url.pathname === "/health") return json(request, env, {
      ok: true,
      product: env.NEXUS_RELAY_NAME || "NEXUS Relay",
      version: "0.5.0",
      model: "hosted metered API with subscription, usage billing, and signed custody receipts",
      supported_modes: [...MODES],
      surfaces: ["REST", "MCP", "SKILL.md", "billing"],
      skill_url: `${url.origin}/SKILL.md`,
      billing_status: env.STRIPE_SECRET_KEY ? "configured" : "not configured",
      egress_status: env.RELAY_EGRESS_URL ? "pinned-dns" : "worker-preflight",
      receipt_signing_status: env.RELAY_RECEIPT_SIGNING_KEY ? "configured" : "not configured",
    });
    ...
    try {
      ...
      if (url.pathname === "/internal/billing/flush" && request.method === "POST") {
        if (!env.BILLING_CRON_SECRET || request.headers.get("x-relay-cron-secret") !== env.BILLING_CRON_SECRET) throw Object.assign(new Error("unauthorized"), { status: 401 });
        return json(request, env, { ok: true, outbox: await flushBillingOutbox(env) });
      }
      ...
      if (url.pathname === "/v1/billing" && request.method === "GET") { const principal = await authenticate(request, env); return json(request, env, { ok: true, billing: await billingSummary(env, principal) }); }
      if (url.pathname === "/v1/billing/checkout" && request.method === "POST") { const principal = await authenticate(request, env); return json(request, env, { ok: true, checkout: await createCheckoutSession(env, principal, await request.json()) }); }
      if (url.pathname === "/v1/billing/portal" && request.method === "POST") { const principal = await authenticate(request, env); return json(request, env, { ok: true, portal: await createPortalSession(env, principal, await request.json()) }); }
  ...
  async scheduled(_controller, env, ctx) { ctx.waitUntil(flushBillingOutbox(env)); },
};

const isPrivateHost = (hostname) => hostname === "localhost" || hostname.endsWith(".local") || isBlockedIp(hostname);
export { detectMode, isPrivateHost, meteredRead, readPublicUrl, sealRelayReceipt, validateUrl };
