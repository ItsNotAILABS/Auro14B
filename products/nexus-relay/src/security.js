...
export function isBlockedIp(value) {
  ...
  if (ip.startsWith("2001:10:")) return true;
...
export function validateUrl(raw) {
  ...
  if (!host || host === "localhost" || host.endsWith(".localhost") || host.endsWith(".local") || isBlockedIp(host)) {
    throw Object.assign(new Error("private, local, or reserved network targets are blocked"), { status: 400 });
  }
...
export async function secureFetch(raw, options = {}) {
  ...
  for (let hop = 0; hop <= maxRedirects; hop += 1) {
    ...
    if (![301, 302, 303, 307, 308].includes(response.status)) return { response, finalUrl: current, redirectChain: chain };
