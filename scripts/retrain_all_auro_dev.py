...
def retrain_model(model_id: str, root: Path, output: Path, steps: int, seq_len: int, max_files: int, max_chars: int) -> dict[str, Any]:
    ...
    cfg = TrainConfig(
        ...
        report_every=max(1, steps),
        multi_repo=False,
        max_corpus_files=max_files,
        max_corpus_chars=max_chars,
        extra={"source_seed": "training/brain_ai_seed_report.md", "promotion": "PROTOTYPE"},
    )
    ...
    restoration_sample = restored.generate(
        "BRAIN-AI deterministic state, browser intelligence, secure execution:",
        max_new_tokens=12,
        temperature=0.0,
    )
    token_path = checkpoint / "tokenizer.json"
    return {
        "model_id": model_id,
        "mode": "dev",
...
def main() -> None:
    ...
    for model_id in requested:
        results.append(retrain_model(model_id, root, out / "checkpoints", args.steps, args.seq_len, args.max_files, args.max_chars))
    ...
    receipt = {
        ...
        "truth_boundary": (
            "This receipt proves executable dev-geometry training and checkpoint restoration only. "
            "It does not claim that the 2B/4B/8B/14B/100B parameter targets were fully trained."
        ),
    }
